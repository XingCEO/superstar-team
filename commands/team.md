...
## Phase 2.5：設計流水線（有前端就必跑，純 API / CLI 跳過）

**判斷邏輯：** 如果本次需求會啟動前端 agent（全端 Web app、純前端、靜態站），就必須跑這步。純 API / CLI / 後端服務跳過。

**如果使用者已經有 DESIGN.md 或設計稿：** 跳過整個 Phase 2.5，直接用現有的。
...
設計槍探索會自動：
1. 根據 DESIGN.md 和架構文件，產出 3-8 個頁面設計變體
2. 開啟對比看板（HTTP 伺服器），使用者直接在瀏覽器上評分、標註
...
**Lead 確認 HTML 品質後進入 Phase 3。**

---

**Phase 2.5 快速模式（小功能或趕時間）：**
...
使用者確認架構、設計系統、設計稿後，Lead 根據 docs/architecture.md 判斷需要哪些 agent：
...
4. Phase 2.5 產出的 HTML/CSS（如果有）— 這是你的起點，不要砍掉重練
...
**這步的目的：在合併前就抓住設計問題，不要等到 Phase 5.4 才發現。**
