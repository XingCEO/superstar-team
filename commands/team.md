...
## API 契約
嚴格按 api-contract.md 實作。有問題回報 Lead（哪個端點、什麼問題、建議改法），不可自改。

## 安全限制
- 3 次同一錯誤 → 停止回報 | 不確定 → 回報不猜
- 套件 >10 / 檔案 >20 → 先回報 | 禁刪檔

## 回報（≤ 300 字）
1. 完成了什麼 2. 檔案清單 3. 測試結果 4. 問題
禁止回報過程。
```
...
## 設計規則
所有視覺決策必須符合 DESIGN.md。開工前必讀 `docs/DESIGN-RULES-COMPACT.md`。
DESIGN.md 沒覆蓋的決策 → 停下來回報 Lead。

## API 契約
嚴格按 api-contract.md 串接，mock data 符合契約 schema。有問題回報，不可自改。
...
## 回報（≤ 300 字）
1. 完成了什麼 2. 檔案清單 3. 設計合規（是/否）4. 測試結果 5. 問題
禁止回報過程。
```
...
## 回報（≤ 300 字）
1. 完成了什麼 2. 檔案清單 3. 測試結果 + 覆蓋率 4. 問題
禁止回報過程。
```
...
## 限制
- 3 次同一錯誤 → 停止回報 | 不改程式碼，只產報告

## 回報（≤ 300 字）
1. 掃描範圍 2. 發現清單（嚴重度 + 一句話）3. 報告在 docs/security-review.md
禁止回報過程。
```
...
### 3. 安全限制已內建
每個 agent prompt 和 agent 定義檔都已包含安全限制（3 次出局 + 套件/檔案上限）。
guardrail.sh hook 在程式碼層面強制執行。不需要額外加。
